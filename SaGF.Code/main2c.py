...
testset = torchvision.datasets.ImageFolder(root='../data/Finetuning2c4TSNE/test_scoffold/', transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=int(args.bs), shuffle=True, num_workers=0)
transf = transforms.ToTensor()


train_after_dataset = torchvision.datasets.ImageFolder(root='../data/Finetuning2c4TSNE/train_scoffold/', transform=transform_test)
# for i in range(1):
...
def test2c(epoch, vit4trainafter, net4trainafter):
    vit4trainafter.eval()
    net4trainafter.eval()
    test_loss = 0
    correct = 0
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            # 获取预测概率（关键修改点）
            ######################################################
            # 修改1：使用softmax代替sigmoid处理多分类输出
            probs = torch.softmax(outputs, dim=1)  # 形状: (batch_size, num_classes)
            positive_probs = probs[:, 1]          # 取正类概率（假设类别1为正类）
            ######################################################
        ...
        scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=colors, s=5, cmap='viridis', alpha=0.7)
    ...
    # 计算整体ROC AUC（关键修改点）
    ######################################################
    # 修改2：在整个测试集上统一计算（不再分批次累加）
    roc_auc = roc_auc_score(all_targets, all_probs)
    ######################################################

    # 计算混淆矩阵
    cm = confusion_matrix(all_targets, (np.array(all_probs) >= 0.5).astype(int))
    ...
    # 输出结果
    content = f'Epoch {epoch}, test loss: {test_loss:.5f}, acc: {acc:.5f}, roc_auc: {roc_auc:.5f}, sensitivity: {sensitivity:.5f}, specificity: {specificity:.5f}'
    print(content)

    return test_loss, acc, roc_auc, sensitivity, specificity
...
def trainandevl():
    ...
    for epoch in range(0, args.n_epochs_tafter):
        ...
        if epoch % 1 == 0:
            test_loss, acc, roc_auc, sensitivity, specificity = test2c(0, vit4trainafter, net4trainafter)
...
for interation in range(args.n_epochs):
    ...
    if interation >= 100 and (biaozhi == 1):  #interation%10 == 0 or
        learner.load_state_dict(torch.load('improved-net.pth'), strict=False)  # -224-0.8056

        # display the TSNE image to test the efficicency of pre-training
        test_loss, acc, roc_auc, sensitivity, specificity = test2c(0, learner.gen_vit, learner.online_encoder.net)
