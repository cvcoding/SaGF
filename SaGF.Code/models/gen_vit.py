...
class Attention_3GIN(nn.Module):
    ...
    def forward(self, x):
        ...
        v = ((rearrange(v, 'b h t d -> b t (h d)', h=self.num_heads)))

        out = v# (self.proj_v(v)) #torch.sigmoid
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout, image_size, patch_size, kernel_size, downsample, batch_size, in_chans,
        ...
        for i in range(self.depth):
            self.layers.append(nn.ModuleList([
                Residual(PreNorm(dim, Attention_3GIN(depth, i, dim, image_size=image_size, patch_size=patch_size,
    ...
    def forward(self, img):
        ...
        for attn, ff in self.layers:
            ...
            x, _ = ff(x)
        return x_pre, x, norm_attn_score
...
class gen_vit(nn.Module):
    def __init__(self, *, image_size, patch_size, kernel_size, downsample, batch_size, num_classes, dim, depth, heads, mlp_dim, patch_stride, patch_pading, in_chans, dropout=0., emb_dropout = 0., expansion_factor=1):
        ...
        self.Upool = nn.Sequential(
            nn.Linear(dim, int(image_size / patch_size/2) ** 2, bias=True),
        )
        self.Upool4x = nn.Sequential(
            nn.Linear(dim, int(image_size / patch_size / 2) ** 2, bias=True),
        )

        self.Upoolrank = nn.Sequential(
        ...
        self.image_newsize = image_size//4

        self.new_size = (self.image_newsize, self.image_newsize)
        self.patchsalow = self.image_newsize // self.patch_size
    ...
    def forward(self, img):
        img = F.interpolate(img, size=self.new_size, mode='bilinear', align_corners=True)
        ...
        x_pre, x, norm_attn_score = self.transformer(img)
        x_pre = rearrange(x_pre.permute(0, 2, 1), 'b c (w h) -> b c w h', w=self.patchsalow, h=self.patchsalow)

        x_pre = self.triattention(x_pre)
        x_pre = rearrange(x_pre, 'b c w h -> b c (w h)').permute(0, 2, 1)

        x_gate = self.proj_gate(x)
        inver_x_gate = 1 - x_gate

        x_lowrank = x_pre * x_gate
        x_sparsenoise = x_pre * inver_x_gate

        temp = self.Upool4x(x_pre).permute(0, 2, 1)
        C = F.normalize(temp, dim=-1)
        x_pre = torch.matmul(C, x_pre)

        rank_k = torch.sum(torch.sigmoid(self.Upoolrank(x_sparsenoise).squeeze()), dim=-1)
        sparsity_v = int(torch.round(torch.mean(rank_k)))

        # batch_size = img.size(0)

        temp = self.Upool(x_sparsenoise).permute(0, 2, 1)
        # C = F.gumbel_softmax(temp, dim=-1, tau=1.0)
        # C = temp # F.softmax(temp/2, dim=-1)
        C = F.normalize(temp, dim=-1)
        x_sparsenoise_pool = torch.matmul(C, x_sparsenoise)

        _, n, _ = x.size()

        #基于low rank生成邻接矩阵
        # Z = torch.sigmoid(torch.matmul(x_lowrank, torch.transpose(x_lowrank, -1, -2)))
        # norm_attn_score = Z.unsqueeze(dim=1).expand(-1, self.heads, -1, -1).to(device)
        #
        # C = C.unsqueeze(dim=1).expand(batch_size, self.heads, -1, -1).to(device)
        # temp2 = torch.matmul(C, norm_attn_score)
        # norm_attn_score = torch.matmul(temp2, C.permute(0, 1, 3, 2))

        Z = torch.sigmoid(torch.matmul(x_sparsenoise, torch.transpose(x_sparsenoise, -1, -2)))
        norm_attn_score = Z.unsqueeze(dim=1).expand(-1, self.heads, -1, -1).to(device)

        # x = self.to_cls_token(x[:, -1])

        return x_pre, x_lowrank, x_sparsenoise, x_sparsenoise_pool, norm_attn_score, sparsity_v
